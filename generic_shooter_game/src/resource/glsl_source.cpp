...
// -- UniformMapping --
gsg::UniformMapping gsg::uniform_mapping_from_name(std::string const& name) {
...
// -- UniformBlockMapping --
gsg::UniformBlockMapping gsg::uniform_block_mapping_from_name(std::string const& name) {
	UMAP(LIGHT_DATA)
...
// -- GLSLSource --
std::string gsg::GLSLSource::uniform_type(UniformMapping map) {
	switch(map) {
	case LOCAL_TO_CLIP:
	case LOCAL_TO_WORLD:
	case WORLD_TO_CLIP:
			...
			for (size_t i_line = 0; i_line < src.size() - 1; ++i_line) {
				...
				if (regex_search(line, result, map_rgx)) {
					...
					if (regex_search(line_below, result, unif_rgx)) {
						...
						gsg::UniformMapping map;
						try {
							map = gsg::uniform_mapping_from_name(map_str);
						}
					...
					// line below is a uniform block
						...
						gsg::UniformBlockMapping block_map;
						try {
							block_map = gsg::uniform_block_mapping_from_name(result_str);
						}
						catch (std::exception const& e) {
							throw std::runtime_error("line " + std::to_string(i_line + 1) + ":\n" + e.what());
						}
...
std::unique_ptr<sol::IResource> gsg::GLSLSource::clone() const {
	return std::unique_ptr<GLSLSource>(new GLSLSource(*this));
}

std::string const& gsg::GLSLSource::vertex_shader() const {
	return m_vertex;
}

std::string const& gsg::GLSLSource::fragment_shader() const {
	return m_fragment;
}

boost::optional<std::string> const& gsg::GLSLSource::geometry_shader() const {
	return m_geometry;
}

boost::optional<std::string> const& gsg::GLSLSource::tess_control_shader() const{
	return m_tess_ctrl;
}

boost::optional<std::string> const& gsg::GLSLSource::tess_eval_shader() const {
	return m_tess_eval;
}

boost::optional<std::string> const& gsg::GLSLSource::compute_shader() const {
	return m_compute;
}

std::unordered_multimap<gsg::UniformMapping, std::string> const& gsg::GLSLSource::mapped_uniforms() const {
	return m_unifs;
}

std::unordered_multimap<gsg::UniformBlockMapping, std::string> const& gsg::GLSLSource::mapped_uniform_blocks() const {
