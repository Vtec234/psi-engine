...
namespace std {
template <>
struct hash<gsg::UniformMapping> {
	size_t operator()(gsg::UniformMapping const& m) const {
		return static_cast<size_t>(m);
	}
};

template <>
struct hash<gsg::UniformBlockMapping> {
	size_t operator()(gsg::UniformBlockMapping const& m) const {
		return static_cast<size_t>(m);
	}
};
} // namespace std
...
public:
	...
	static std::unique_ptr<sol::IResource> construct_from_sources(std::array<std::vector<std::string>, 6> const& sources);

	~GLSLSource() = default;

	std::unique_ptr<sol::IResource> clone() const override;
