...
std::unique_ptr<sol::IWindowService> gsg::GLWindowService::start_gl_window_service(GLWindowServiceArgs args) {
	...
	glfwSetErrorCallback(
		[] (GLint code, char const* desc) {
			log::error("GLFW") << "(" << code << "): " << desc << "\n";
		}
	);

	log::info("GLWindowService") << "Initialized GLFW " << glfwGetVersionString() << "\n";
...
void gsg::GLWindowService::update_window() {
...
void gsg::GLWindowService::set_mouse_block(bool block) {
	if (block)
...
bool gsg::GLWindowService::should_close() const {
	return m_should_close;
}

uint32_t gsg::GLWindowService::width() const {
	return m_framebuffer_width;
}

uint32_t gsg::GLWindowService::height() const {
	return m_framebuffer_height;
}

double gsg::GLWindowService::aspect_ratio() const {
	return m_aspect_ratio;
}

void gsg::GLWindowService::framebuffer_size_callback(int width, int height) {
...
void gsg::GLWindowService::key_pressed_callback(int key, int scancode, int action, int mods) {}

void gsg::GLWindowService::mouse_pressed_callback(int button, int action, int mods) {}

void gsg::GLWindowService::cursor_moved_callback(double x, double y) {}
